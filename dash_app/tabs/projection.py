...
# import CSV
df_sex = pd.read_csv(
    'https://epistat.sciensano.be/Data/COVID19BE_CASES_AGESEX.csv', sep=',', encoding='latin-1')
df_hosp = pd.read_csv(
    'https://epistat.sciensano.be/Data/COVID19BE_HOSP.csv', sep=',', encoding='latin-1')
...
for i in range(len(df_proj['DATE']) - 1):
    for x in range(len(df_measures["DATE"]) - 1):
        if (df_proj['DATE'].iloc[i] >= df_measures['DATE'].iloc[x]) & (
            ...
            df_proj["Growth Factor"].iloc[i] = df_measures["R0"].iloc[x] / \
                avg_contagious_period
